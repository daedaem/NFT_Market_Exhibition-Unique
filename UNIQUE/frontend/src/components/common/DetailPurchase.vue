<template>
  <div>
    <button type="button" data-bs-toggle="modal" data-bs-target="#modal" class="btn btn-sm btn-dark d-block">Purchase</button>
    <!-- <button type="button" data-bs-toggle="modal" :data-bs-target="`#modal${product.nft.nftSeq}`" class="btn btn-sm btn-dark d-block">Purchase</button> -->
    ...
    <!-- end firstmodal -->
    <!-- start second modal -->
    <div class="modal fade" id="moda1" tabindex="-1" aria-hidden="true" v-if="authorPrivateKey">
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          <div class="modal-header d-flex flex-column">
            <h4 class="modal-title d-flex justify-content-center">You created {{ nftName }}</h4>
...
export default {
  ...
  methods: {
    ...
    //   let aca = await NFTContractInstance.methods.ownerOf(this.nftTokenId).call();
    //   // let csc = await NFTContractInstance.methods.ownerOf(33).call();
    //   let vas = await NFTContractInstance.methods.getApproved(this.nftTokenId).call();
    //   console.log(aca, "aca");
    //   // console.log(csc, "csc");
    //   console.log(vas, "vas");
      ...
      if (checkPubKey === myAccount) {
        ...
        if (myTokenBalance > this.price) {
          ...
          // 서명할게 있으면
            ...
            // let checking = NFTContractInstance.methods.getApproved(this.nftTokenId).call();
  ...
  mounted() {
    // console.log(this.product, "아");
  },
  created: function () {
    // this.check();
    // console.log(this.nowInfo, "아");
    console.log(this.nftCA, "onsale");
    // console.log(this.product, "아");
