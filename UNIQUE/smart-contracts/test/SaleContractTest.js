...
contract("Sale Contract Testing", (accounts) => {
  ...
  it("Purchase", async () => {
    ...
    const mint = await ssafyTokenContract.mint(mintAmount);
    ...
    // NFT 판매 함수 호출
    await salesFactoryContract.createSale(TokenId, 100, startTime, endTime, currencyAddress, nftAddress);
    ...
    const resulttwo = await salesContract.purchase({ from: purchaser });
    // const resulttwo = await salesContract.purchase()({ from: purchaser });

    // 여기 어째하냐 구매자 지정해서 호출 어떻게...?
    // const resultPurchase = salesContract.purchase();
    // console.log(resulttwo, "이거 뭔데");
    // const resultPurchase = await salesContract.purchase();
    // const purchaser =

    // 확인사항
    // 최종 NFT소유자가 구매자.
    // console.log(await web3.eth.getBalance(purchaser));
    // 구매자의 잔액이 900과 같다.
    // const getNftOwner = async (TokenId) => {
    //   await SsafyNFT.ownerOf(TokenId)
    //   }
    // TODO
    // 다음을 테스트를 통과해야합니다.
    // console.log(await salesFactoryContract.allSales.call());
